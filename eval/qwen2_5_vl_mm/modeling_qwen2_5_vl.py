...
class Qwen2_5_VLFlashAttention2(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        ## [M2] Compute the text guided attention weights
        ...
        if output_attentions and self.video_end_pos is not None:
            q_len = query_states.shape[2]
            k_len = key_states.shape[2]
            if q_len == k_len:
                # prefilling
                # bsz, head_nums, seq_len , head_dim
                window_size = q_len - self.video_end_pos - 1
                assert (
                    window_size > 0
                ), f"window_size must be greater than 0, q_len: {q_len}, video_end_pos: {self.video_end_pos}, k_len: {k_len}"

            else:
                assert q_len == 1, f"q_len must be 1 during decoding q_len: {q_len}"
                ## decoding
                window_size = 1

            text_query_states = query_states[:, :, -window_size:, :]
            attn_weights = torch.matmul(
                text_query_states, key_states.transpose(-1, -2)
            ) / math.sqrt(self.head_dim)

            causal_mask = generate_causal_mask(
                text_query_states.shape[2], self.num_heads
            ).to(attn_weights.device)

            attn_weights[:, :, :, -window_size:] = (
                attn_weights[:, :, :, -window_size:] + causal_mask
            )
            attn_weights = torch.nn.functional.softmax(
                attn_weights, dim=-1, dtype=torch.float32
            ).to(input_dtype)
            attn_weights = attn_weights.max(dim=-2, keepdim=True).values
        else:
            attn_weights = None
        ## END
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    ...
    # @property
    # def video_end_pos(self):
    #     return getattr(self.self_attn, "video_end_pos", None)

    # @video_end_pos.setter
    # def video_end_pos(self, idx: int):
    #     self.self_attn.video_end_pos = idx
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    @property
    def video_end_pos(self):
        return getattr(self.layers[0].self_attn, "video_end_pos", None)

    @video_end_pos.setter
    def video_end_pos(self, idx: int):
        for layer in self.layers:
            layer.self_attn.video_end_pos = idx
...
class Qwen2_5_VLForConditionalGeneration(GenerationMixin, Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        selected_visual_token_idx: Optional[torch.Tensor] = None,
        ## End
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                ## [M2] directly pass video_embeds for efficiency
                ...
                ## [M2]
                video_end_pos = torch.where(mask == True)[1][-1].item()
                video_start_pos = torch.where(mask == True)[1][0].item()

                self.model.video_end_pos = video_end_pos
                if selected_visual_token_idx is not None:
                    prefix_index = torch.arange(
                        video_start_pos, device=input_ids.device
                    )
                    self.model.video_end_pos = None
                    suffix_index = (
                        torch.arange(
                            input_ids.shape[1] - video_end_pos - 1,
                            device=input_ids.device,
                        )
                        + video_end_pos
                        + 1
                    )
                    selected_token_idx = torch.cat(
                        [prefix_index, selected_visual_token_idx+prefix_index.shape[0], suffix_index], dim=0
                    )
                ## END
        ...
        if selected_token_idx is not None:
            inputs_embeds = inputs_embeds[:, selected_token_idx, :]
            attention_mask = attention_mask[:, selected_token_idx]
            position_ids = position_ids[:, :, selected_token_idx]

        pass
        ##
        # print("input embedding shape:", inputs_embeds.shape)
        outputs = self.model(
            input_ids=None,
            position_ids=position_ids,
            attention_mask=attention_mask,
        ...
        # print(f"Qwen2_5_VLForConditionalGeneration forward time: {time_elapsed } s")
        ...
        **model_kwargs,
        ...
        def _expand_dict_for_generation_visual(dict_to_expand):
            ...
            for key in dict_to_expand:
                ...
                ##[M2]
                elif key == "video_embeds":
                    # raise NotImplementedError(
                    #     "expand_size > 1 is not supported when passing video_embeds. Please pass pixel_values_videos and video_grid_thw instead."
                    # )
                    samples = torch.split(video_grid_thw, list(video_nums))
                    lengths = [
                        torch.prod(sample, dim=1).sum() // 4 for sample in samples
                    ]
                    dict_to_expand[key] = _repeat_interleave_samples(
                        dict_to_expand[key], lengths=lengths, repeat_times=expand_size
                    )
