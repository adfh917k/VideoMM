...
def _read_video_decord(
    ...
    video = extract_frames(video_path, idx)

    # output_dir = f"/xikexie/ghy/video_m2/eval/saved_frames/saved_frames_{video_path.split('/')[-1][:-4]}"  # 你可以改成以视频名命名的文件夹
    # os.makedirs(output_dir, exist_ok=True)

    # print(f"正在保存 {len(video)} 张图片到 {output_dir} ...")

    # if os.listdir(output_dir):  # 如果文件夹非空
    #     print(f"文件夹 {output_dir} 已存在且非空，跳过保存！")
    # else:
    #     print(f"正在保存 {len(video)} 张图片到 {output_dir} ...")
    ...
    # for i, frame_data in enumerate(video):
    #     # frame_data: 单帧数据
    #     # idx[i]: 这一帧在原视频中的真实索引号
        
    #     # 兼容性处理：如果你用的 extract_frames 返回的是 Tensor，转成 numpy
    #     if hasattr(frame_data, 'cpu'):
    #         frame_data = frame_data.cpu().numpy()
...
def extract_frames(video_path, idx, num_processes=8):
    from multiprocessing import Pool
    import numpy as np

    # 分块
    chunk_size = len(idx) // num_processes
    chunks = [idx[i : i + chunk_size] for i in range(0, len(idx), chunk_size)]
    args = [(video_path, chunk) for chunk in chunks]
...
def fetch_video(model, ele: Dict[str, Any], image_patch_size: int = 16, return_video_sample_fps: bool = False,
    ...
    with model.gpu_lock:
        ...
        def _resize_in_batches(
            x: torch.Tensor, size_hw: list[int], bs: int
        ) -> torch.Tensor:
            """Resize a 4D video tensor (T, C, H, W) in batches along T to lower peak memory."""
            T = x.shape[0]
            chunks = []
            for start in range(0, T, bs):
                ...
                chunks.append(chunk_resized)
            return torch.cat(chunks, dim=0)

        # Resize high-res in batches, then move to CPU to free GPU memory before low-res
        video_high_gpu = _resize_in_batches(
            video, [resized_height, resized_width], batch_size
        )

        # Resize low-res in batches, then move to CPU
        video_medium_gpu = _resize_in_batches(
            video, [resized_height // 2, resized_width // 2], batch_size
        )

        video_low_gpu = _resize_in_batches(
            video, [resized_height // 4, resized_width // 4], batch_size
        )
        del video

        video_high_cpu = video_high_gpu.float().cpu()
        video_medium_cpu = video_medium_gpu.float().cpu()
        video_low_cpu = video_low_gpu.float().cpu()
    ...
    #     antialias=True,
    # ).float()
    video = (video_high_cpu, video_medium_cpu, video_low_cpu)
...
def process_vision_info(
    model, conversations: Union[List[Dict[str, Any]], List[List[Dict[str, Any]]]],
    ...
    for vision_info in vision_infos:
        if "image" in vision_info or "image_url" in vision_info:
            ...
            video_input, video_sample_fps = fetch_video(model, vision_info, return_video_sample_fps=True,
